...
S0_REF = REF_S0
LW_REF = REF_LW
AMP_NOM = dict(A_over_m=REF_A_OVER_M, omega_star=REF_OMEGA_STAR,
...
def q_star(p):
    return p.aw_over_m * s0_of(p.Lw, p.phi1) ** 2 * p.omega_star ** 2
...
def main():
    ...
    for k in range(n_pts):
        p = replace(base, A_over_m=N_WINGS * Aw[k], omega_star=om[k], phi1=ph[k], Lw=lw[k],
    ...
    axL.set_xlabel(r"$q^\ast = \frac{1}{4} (A^\ast/m^\ast)\, s_0^2\, \omega^{\ast 2}$")
