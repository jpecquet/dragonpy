...
Parameter naming mirrors Table 1 of the report:
    ...
    A_over_m    inverse wing loading         N_w A_w rho L / m   (total over all wings)
...
A_over_m is the TOTAL inverse wing loading of Table 1 (all N_w wings). The
per-wing value the force law actually needs is A_over_m / N_w, exposed as the
`Params.aw_over_m` property. The analysis quantities (q*, the feasibility
criterion) are computed from it, but the report text and figure axes present
them in the total A*/m* of Table 1 (q* = (1/4)(A*/m*)(s0 omega)^2).
...
N_WINGS = 4   # fore + hind, left + right; sets per-wing area = A_over_m / N_WINGS
...
REF_A_OVER_M = 0.6   # total inverse wing loading; per-wing A_w*/m* = 0.6 / 4 = 0.15
REF_OMEGA_STAR = 14.0
REF_LW = 0.75
REF_S0 = 0.25
...
class Params:
    ...
    A_over_m: float = 0.6       # total inverse wing loading N_w A_w rho L / m, [0.2, 1.0]
    ...
    @property
    def aw_over_m(self) -> float:
        """Per-wing inverse wing loading A_w rho L / m = A_over_m / N_w.

        This is what the single-wing force law and all the per-wing analysis
        quantities (q*, the feasibility criterion, the figure axes) use.
        """
        return self.A_over_m / N_WINGS
...
def make_pattern(p: Params) -> StrokePattern:
    ...
    return StrokePattern(
        # Report convention: stroke-plane normal n = cos(gamma0) z - sin(gamma0) x,
        # so +gamma0 tilts the normal (and the symmetric-pitch force) toward -x.
        # dragonpy's stroke_plane_tilt has the opposite sense, hence the negation.
...
def _wing_force_vec(p: Params, chirality: int, H: np.ndarray,
    ...
    # +gamma0 tilts the normal toward -x (report convention n = cos g z - sin g x);
    # the in-plane tilt rotation is about +x, so the body-frame tilt is +chirality*gamma0.
...
__all__ = [
    ...
    "STUDY_SPAN_FRAC", "STUDY_ELEMENT", "N_WINGS",
    "REF_A_OVER_M", "REF_OMEGA_STAR", "REF_LW", "REF_S0",
]
