...
from feasibility import (
    Params, REF_AW_OVER_MB, REF_LW, REF_OMEGA_STAR, STUDY_ELEMENT,
...
DELTA0 = np.pi / 2.0
AW_OVER_MB, OMEGA_STAR, LW = REF_AW_OVER_MB, REF_OMEGA_STAR, REF_LW
...
def base_params(**over):
    return replace(Params(gamma0=GAMMA0, psi1=PSI1, delta0=DELTA0,
                          Aw_over_mb=AW_OVER_MB, omega_star=OMEGA_STAR, Lw=LW,
...
def solve_equilibrium(p, mass_ratio=MASS_RATIO):
    ...
    for _ in range(8):
        ...
        # Direction: F_x decreases as psi0 grows (more aft tilt -> less +x).
        psi0 = _bisect(
            lambda v: float(cycle_averaged_force(replace(p, psi0=v), N_PHASE)[0]),
            np.radians(-10.0), np.radians(70.0), target=0.0, increasing=False)
