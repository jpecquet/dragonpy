...
hold a fixed operating point and map F_a^* in the (A_w*/m_b*, omega*) plane.
...
Axes:
    A_w*/m_b*  per wing, [0.0075, 0.035] / 4 = [0.001875, 0.00875]
    omega*     [5, 17]
...
def main():
    ...
    n = 120
    aero = np.linspace(0.0075, 0.035, n) / 4.0      # per-wing inverse wing loading
        ...
        for j, w in enumerate(omega):
            for i, a in enumerate(aero):
                Z[j, i] = avg_force_magnitude(
                    replace(BASE, Aw_over_mb=a, omega_star=w,
    ...
    # x-axis is the total inverse wing loading A*/m_b* = 4 * (per-wing), to match
    # the plot we compare against; the model still uses the per-wing value.
    X, Y = np.meshgrid(4.0 * aero, omega)
