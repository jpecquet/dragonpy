"""
Sweep the wing span_ratio (spanwise length / body length) at fixed pursuit
gain k_tilt = 3, intercept feather amplitude = 30 deg, and aero_ratio = 0.025,
running a capture study for each value and generating the half-disk plots.
...
The span_ratio = 0.75 baseline study already exists at
...
def study_tag(
    ...
    intercept_feather_amp: float, span_ratio: float,
) -> str:
    feather_deg = int(round(np.degrees(intercept_feather_amp)))
    span_tag = f"{int(round(span_ratio * 1e3)):04d}"
...
def run_one(
    ...
    span_ratio: float, n_workers: int,
):
    sensing_delay = T_WB
    hind_phase = np.pi / 2
    ...
    label = f"k={k_tilt}, span={span_ratio:g}"
    ...
    tag = study_tag(k_tilt, sensing_delay, hind_phase,
                    intercept_feather_amp, span_ratio)
...
if __name__ == "__main__":
    ...
    for span in span_ratios:
        result, out_dir = run_one(
            k_tilt, intercept_feather_amp, aero_ratio, span, n_workers,
        )
        studies.append((span, result, out_dir))

    for span, result, out_dir in sorted(studies, key=lambda t: t[0]):
        print(f"plotting span={span:g}...", flush=True)
