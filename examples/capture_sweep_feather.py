"""
Sweep the intercept feather amplitude across multiple values at fixed pursuit
gain k_tilt = 3, running a capture study for each and generating the half-disk
plots. The full-sphere FOV means hover mode is never engaged, so only the
intercept-mode kinematics matter for the capture rate.
...
def run_one(k_tilt: float, intercept_feather_amp: float, n_workers: int):
    sensing_delay = T_WB
    hind_phase = np.pi / 2
    ...
    feather_deg = np.degrees(intercept_feather_amp)
    label = f"k={k_tilt}, feath={feather_deg:.0f}deg"
    ...
    tag = study_tag(k_tilt, sensing_delay, hind_phase, intercept_feather_amp)
...
if __name__ == "__main__":
    ...
    feather_amps_deg = (15.0, 45.0, 60.0)
    studies: list[tuple[float, object, Path]] = []

    for feath_deg in feather_amps_deg:
        feather_amp = np.radians(feath_deg)
        result, out_dir = run_one(k_tilt, feather_amp, n_workers)
        studies.append((feath_deg, result, out_dir))

    for feath_deg, result, out_dir in sorted(studies, key=lambda t: t[0]):
        print(f"plotting feath={feath_deg:.0f}deg...", flush=True)
