"""
XZ-plane trajectory plot for a K=3, tau/T_wb=1 controller pursuing
stationary prey at (x, z) = (+3, +3) and (+3, -3), comparing fore/hind
wing phase offsets 0 and pi.
...
OUT_STEM = "pursuit_k_tau_phase_trajectories"

PHASES = [
    (0.0,   "--", r"$\Delta\phi = 0$"),
    (np.pi, "-",  r"$\Delta\phi = \pi$"),
]
...
def plot_trajectories(runs, savepath, theme):
    ...
    target_colors = {
        +3.0: style.trajectory_color,
        -3.0: style.error_color,
    }
    ...
    for entry in runs:
        ...
        color = target_colors[float(prey[2])]
    ...
    handles = [
        Line2D([0], [0], color=style.text_color, linestyle=ls, linewidth=style.trajectory_linewidth, label=label)
        for _, ls, label in PHASES
    ]

    ax.set_title(r"$K = 3$,  $\tau/T_\mathrm{wb} = 1$")
...
def main():
    ...
    runs = []
    for phase, linestyle, label in PHASES:
        for prey in preys:
            r = run_case(
                f"k3_tau1Twb_x{prey[0]:g}_z{prey[2]:g}_phase{phase:.3f}",
                k_tilt=3.0,
                sensing_delay=T_wb,
                prey_position=prey,
                hind_phase_offset=float(phase),
                t_end=10.0,
                fov_half_angle=np.pi,
            )
            runs.append({"result": r, "linestyle": linestyle, "phase": phase})
            print(f"phase={phase:.3f}  prey ({prey[0]:+g}, {prey[2]:+g}): "
